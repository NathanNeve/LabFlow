<script lang="ts">
	...
	import { fetchMicrobiologyCatalogVoedingsbodems } from '$lib/fetchFunctions';
	// @ts-ignore
	...
	let token: string = '';
	let searchCode = '';
	let items: MicrobiologyVoedingsbodem[] = [];
	let itemsSorted: MicrobiologyVoedingsbodem[] = [];
	let naam = '';
	let deleteError = '';
	...
	async function reload() {
		const result = await fetchMicrobiologyCatalogVoedingsbodems();
		if (result) {
	...
	async function deleteItem(id: number) {
		try {
			const response = await fetch(`${backend_path}/api/microbiology/voedingsbodems/${id}`, {
				method: 'DELETE',
				headers: {
			...
			if (response.ok) {
				...
				deleteError =
					'Voedingsbodem kon niet worden verwijderd omdat deze gelinked is aan één of meerdere tests of stalen.';
			}
		} catch (error) {
			console.error('Voedingsbodem kon niet worden verwijderd: ', error);
		}
	}
	...
	async function nieuwItem() {
		...
		try {
			const response = await fetch(`${backend_path}/api/microbiology/voedingsbodems`, {
				method: 'POST',
				headers: {
			...
			if (!response.ok) {
				errorMessagePOST = 'Voedingsbodem kon niet worden aangemaakt.';
				return;
			}
			naam = '';
			errorMessagePOST = '';
			...
			console.error('Voedingsbodem kon niet worden aangemaakt: ', error);
		}
	}
	...
	async function updateItem(id: number) {
		...
		try {
			...
			console.error('Voedingsbodem kon niet worden aangepast: ', error);
		}
	}
</script>
...
<div class="flex flex-col w-full ml-5">
	<div class="flex flex-row justify-between w-full h-14 mb-5">
		<h1 class="font-bold text-3xl">Voedingsbodems beheren</h1>
		<button
			type="button"
			on:click={async () => {
		...
		<div class="grid grid-cols-7 space-x-3 my-3 bg-white rounded-lg h-20 items-center px-3 shadow-md">
			<div class="col-span-6">
				<input
					type="text"
					bind:value={naam}
					placeholder="Naam van de voedingsbodem"
			...
			<div class="col-span-1 flex justify-end">
				<button
					...
					on:click={nieuwItem}
					aria-label="Nieuwe voedingsbodem toevoegen"
				>
					<FaPlus />
				</button>
			</div>
