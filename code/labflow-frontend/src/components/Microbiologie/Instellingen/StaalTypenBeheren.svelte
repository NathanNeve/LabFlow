<script lang="ts">
	...
	import { fetchMicrobiologyStaalTypes } from '$lib/fetchFunctions';
	// @ts-ignore
	...
	import type { MicrobiologyStaalType } from '$lib/types/dbTypes';

	const backend_path = import.meta.env.VITE_BACKEND_PATH;

	let token: string = '';
	let searchCode = '';
	let items: MicrobiologyStaalType[] = [];
	let itemsSorted: MicrobiologyStaalType[] = [];
	let naam = '';
	let deleteError = '';
	...
	async function reload() {
		const result = await fetchMicrobiologyStaalTypes();
		if (result) {
	...
	async function deleteItem(id: number) {
		try {
			const response = await fetch(`${backend_path}/api/microbiology/staal-types/${id}`, {
				method: 'DELETE',
				headers: {
			...
			if (response.ok) {
				...
				deleteError =
					'Staaltype kon niet worden verwijderd omdat deze gelinked is aan één of meerdere tests of stalen.';
			}
		} catch (error) {
			console.error('Staaltype kon niet worden verwijderd: ', error);
		}
	}
	...
	async function nieuwItem() {
		...
		try {
			const response = await fetch(`${backend_path}/api/microbiology/staal-types`, {
				method: 'POST',
				headers: {
			...
			if (!response.ok) {
				errorMessagePOST = 'Staaltype kon niet worden aangemaakt.';
				return;
			}
			naam = '';
			errorMessagePOST = '';
			...
			console.error('Staaltype kon niet worden aangemaakt: ', error);
		}
	}
	...
	async function updateItem(id: number) {
		...
		try {
			...
			console.error('Staaltype kon niet worden aangepast: ', error);
		}
	}
</script>
...
<div class="flex flex-col w-full ml-5">
	<div class="flex flex-row justify-between w-full h-14 mb-5">
		<h1 class="font-bold text-3xl">Staaltypen beheren</h1>
		<button
			type="button"
			on:click={async () => {
		...
		<div class="grid grid-cols-7 space-x-3 my-3 bg-white rounded-lg h-20 items-center px-3 shadow-md">
			<div class="col-span-6">
				<input
					type="text"
					bind:value={naam}
					placeholder="Naam van het staaltype"
			...
			<div class="col-span-1 flex justify-end">
				<button
					...
					on:click={nieuwItem}
					aria-label="Nieuw staaltype toevoegen"
				>
					<FaPlus />
				</button>
			</div>
